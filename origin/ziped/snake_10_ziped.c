...
static int astar_state(Game*game,const int sr[],const int sc[],int len,int step0,int dir0,int tgtr,int tgtc,int pathr[],int pathc[],int*fdir);
static int sim_follow_state(Game*game,const int in_sr[],const int in_sc[],int in_len,int step0,int food_tr,int food_tc,int pathr[],int pathc[],int plen,int out_sr[],int out_sc[],int*out_len,int*out_dir);
static int state_space(const Game*game,const int sr[],const int sc[],int len);
static int state_options(const Game*game,const int sr[],const int sc[],int len,int dir0,int step0);
...
static int flood_eval(const Game*game,int startr,int startc,int blk[ROWS][COLS],int tailr,int tailc,int*tail_ok);
static int choose_direction_low(Game*game);
static int safe_after_path(Game*game,const int in_sr[],const int in_sc[],int in_len,int step0,int food_tr,int food_tc,int pathr[],int pathc[],int plen);
static int sim_one_step(const Game*game,int d,int out_sr[],int out_sc[],int*out_len,int*eating_out,int*growing_out);
...
static int sim_follow_global(Game*game,int pathr[],int pathc[],int plen){
int lastdir;
return sim_follow_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->foodr,game->foodc,pathr,pathc,plen,game->tmp_sr,game->tmp_sc,&game->tmp_slen,&lastdir);
}
static int sim_safe_like(const Game*game,int min_space){
int blk[ROWS][COLS];
int vis[ROWS][COLS];
...
safe_food_dir=-1;
fdir=-1;
plen=astar_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->curdir,game->foodr,game->foodc,pathr,pathc,&fdir);
if(plen>0&&fdir>=0){
...
if(safe_food_dir<0&&static_dead_cell(game,game->foodr,game->foodc)&&sim_follow_global(game,pathr,pathc,plen)){
return fdir;
}
}
tail_chase_dir=-1;
if(game->nval>1){
tdir=-1;
astar_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->curdir,game->srow[game->slen-1],game->scol[game->slen-1],pathr,pathc,&tdir);
if(tdir>=0){
tail_chase_dir=tdir;
}
}
...
if(!sim_follow_state(game,in_sr,in_sc,in_len,step0,food_tr,food_tc,pathr,pathc,plen,game->tmp_sr,game->tmp_sc,&game->tmp_slen,&lastdir)){
return 0;
}
if(lastdir<0){
return 0;
...
dummy=-1;
tail_len=astar_state(game,game->tmp_sr,game->tmp_sc,game->tmp_slen,end_step,lastdir,game->tmp_sr[game->tmp_slen-1],game->tmp_sc[game->tmp_slen-1],backr,backc,&dummy);
if(tail_len<=0){
return 0;
}
if(game->nval<=2){
...
if((game->nval>=512&&game->stepcnt>7000)||(game->nval>=256&&elapsed>320)||(game->nval>=128&&elapsed>340)){
...
int food_len;
fdir=-1;
food_len=astar_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->curdir,game->foodr,game->foodc,pathr,pathc,&fdir);
if(food_len>0&&fdir>=0&&safe_after_path(game,game->srow,game->scol,game->slen,game->stepcnt,game->foodr,game->foodc,pathr,pathc,food_len)){
int direct_need;
int direct_space;
if(game->nval<=16){
direct_need=10;
...
if(food_len>0&&fdir>=0&&(static_open_degree(game,game->foodr,game->foodc)<=1||static_dead_cell(game,game->foodr,game->foodc))&&sim_follow_global(game,pathr,pathc,food_len)){
game->planlen=food_len;
game->planpos=1;
game->plantargetr=game->foodr;
game->plantargetc=game->foodc;
...
if(game->nval>1){
dummy=-1;
tail_len=astar_state(game,nsr,nsc,nlen,game->stepcnt+1,d,nsr[nlen-1],nsc[nlen-1],pathr,pathc,&dummy);
}
food_len=0;
food_safe=0;
if(!eating&&game->foodr>=0){
dummy=-1;
food_len=astar_state(game,nsr,nsc,nlen,game->stepcnt+1,d,game->foodr,game->foodc,pathr,pathc,&dummy);
if(food_len>0){
food_safe=safe_after_path(game,nsr,nsc,nlen,game->stepcnt+1,game->foodr,game->foodc,pathr,pathc,food_len);
}
}
fdist=(game->foodr>=0)?mhdist(nsr[0],nsc[0],game->foodr,game->foodc):40;
val=0;
