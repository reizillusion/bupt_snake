...
#define MAXSN 400
#define HCAP 1601
static const int drow[4]={
-1,0,1,0}
;
static const int dcol[4]={
0,-1,0,1}
;
...
static int sim_one_step(int d,int out_sr[],int out_sc[],int*out_len,int*eating_out,int*growing_out);
...
int pr,pc,cr,cc;
srow[0]=scol[0]=-1;
for(i=0;i<ROWS;i++)for(j=0;j<COLS;j++)if(grid[i][j]=='H'){
srow[0]=i;
scol[0]=j;
}
slen=1;
...
if(grid[i][j]=='#'||grid[i][j]=='O')wallmap[i][j]=1;
}
visitcnt[srow[0]][scol[0]]=1;
}
static int astar_state(const int sr[],const int sc[],int len,int step0,int dir0,int tgtr,int tgtc,int pathr[],int pathc[],int*fdir){
int occ[ROWS][COLS];
...
for(d=0;d<4;d++)if(hr+drow[d]==pathr[0]&&hc+dcol[d]==pathc[0]){
*fdir=d;
break;
}
return plen;
...
static int sim_follow_state(const int in_sr[],const int in_sc[],int in_len,int step0,int food_tr,int food_tc,int pathr[],int pathc[],int plen,int out_sr[],int out_sc[],int*out_len,int*out_dir){
int occ[ROWS][COLS];
int len,i,step;
for(i=0;i<in_len;i++){
out_sr[i]=in_sr[i];
...
for(i=0;i<len;i++)occ[out_sr[i]][out_sc[i]]=1;
*out_dir=-1;
...
if(!growing)occ[out_sr[len-1]][out_sc[len-1]]=0;
if(wallmap[nr][nc]||occ[nr][nc])return 0;
occ[nr][nc]=1;
if(growing){
for(i=len;i>0;i--){
out_sr[i]=out_sr[i-1];
...
out_sr[0]=nr;
out_sc[0]=nc;
for(i=0;i<4;i++)if(pr+drow[i]==nr&&pc+dcol[i]==nc){
*out_dir=i;
break;
}
}
...
for(i=0;i<len;i++)blk[sr[i]][sc[i]]=1;
blk[sr[0]][sc[0]]=0;
...
static int state_options(const int sr[],const int sc[],int len,int dir0,int step0){
int occ[ROWS][COLS];
int d,cnt;
build_occ_map(sr,sc,len,occ);
cnt=0;
...
else if(wallmap[nr][nc]||occ[nr][nc]>=0)cnt++;
}
return cnt;
}
static int sim_follow_global(int pathr[],int pathc[],int plen){
int lastdir;
return sim_follow_state(srow,scol,slen,stepcnt,foodr,foodc,pathr,pathc,plen,tmp_sr,tmp_sc,&tmp_slen,&lastdir);
}
...
int tok,tr,tc;
memcpy(blk,wallmap,sizeof(blk));
for(i=0;i<tmp_slen;i++)blk[tmp_sr[i]][tmp_sc[i]]=1;
...
int nr=tr+drow[d];
int nc=tc+dcol[d];
if(nr>=0&&nr<ROWS&&nc>=0&&nc<COLS&&vis[nr][nc])tok=1;
}
if(nval<=1)return cnt>=min_space;
return tok&&cnt>=min_space;
}
static int flood_eval(int startr,int startc,int blk[ROWS][COLS],int tailr,int tailc,int*tail_ok){
int vis[ROWS][COLS];
int qr[ROWS*COLS],qc[ROWS*COLS];
...
int nr=tailr+drow[d];
int nc=tailc+dcol[d];
if(nr>=0&&nr<ROWS&&nc>=0&&nc<COLS&&vis[nr][nc])*tail_ok=1;
}
return cnt;
}
...
safe_food_dir=-1;
fdir=-1;
plen=astar_state(srow,scol,slen,stepcnt,curdir,foodr,foodc,pathr,pathc,&fdir);
if(plen>0&&fdir>=0){
if(sim_follow_global(pathr,pathc,plen)){
extra=(plen>15)?plen/3:0;
if(sim_safe_like(threshold+extra))safe_food_dir=fdir;
}
}
tail_chase_dir=-1;
if(nval>1){
tdir=-1;
astar_state(srow,scol,slen,stepcnt,curdir,srow[slen-1],scol[slen-1],pathr,pathc,&tdir);
if(tdir>=0)tail_chase_dir=tdir;
}
bestd=-1;
bestval=-1;
for(d=0;d<4;d++){
...
memcpy(blk,wallmap,sizeof(blk));
for(i=0;i<slen-1;i++)blk[srow[i]][scol[i]]=1;
if(growing)blk[srow[slen-1]][scol[slen-1]]=1;
blk[nr][nc]=0;
if(growing){
tailr=srow[slen-1];
tailc=scol[slen-1];
...
static int safe_after_path(const int in_sr[],const int in_sc[],int in_len,int step0,int food_tr,int food_tc,int pathr[],int pathc[],int plen){
int backr[ROWS*COLS],backc[ROWS*COLS];
int end_step,lastdir,space,opts,need,dummy,tail_len;
if(plen<=0)return 0;
if(!sim_follow_state(in_sr,in_sc,in_len,step0,food_tr,food_tc,pathr,pathc,plen,tmp_sr,tmp_sc,&tmp_slen,&lastdir))return 0;
...
dummy=-1;
tail_len=astar_state(tmp_sr,tmp_sc,tmp_slen,end_step,lastdir,tmp_sr[tmp_slen-1],tmp_sc[tmp_slen-1],backr,backc,&dummy);
...
if(nval<=4)return choose_direction_low();
bestd=-1;
bestval=-1000000000;
for(d=0;d<4;d++){
...
int dummy,fdist,val;
if(!sim_one_step(d,nsr,nsc,&nlen,&eating,&growing))continue;
...
if(nval>1){
dummy=-1;
tail_len=astar_state(nsr,nsc,nlen,stepcnt+1,d,nsr[nlen-1],nsc[nlen-1],pathr,pathc,&dummy);
}
food_len=0;
food_safe=0;
if(!eating&&foodr>=0){
dummy=-1;
food_len=astar_state(nsr,nsc,nlen,stepcnt+1,d,foodr,foodc,pathr,pathc,&dummy);
if(food_len>0)food_safe=safe_after_path(nsr,nsc,nlen,stepcnt+1,foodr,foodc,pathr,pathc,food_len);
}
...
if(tail_len>0)val+=11000+(tail_len<80?tail_len:80)*25;
else val-=3000;
if(food_safe)val+=24000-food_len*((fill_level>60)?90:45);
else if(food_len>0)val+=2200-food_len*12;
if(eating){
...
if(hunger>40&&food_len>0)val+=(hunger-40)*(food_safe?120:25);
if(hunger>100&&eating)val+=6000;
if(hunger>140&&food_len<=0&&!eating)val-=4000;
if(val>bestval){
bestval=val;
bestd=d;
}
