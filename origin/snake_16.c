...
static void rebuild_bodyid(Game*game,const int sr[],const int sc[],int len);
static void build_occ_map(const int sr[],const int sc[],int len,int occ[ROWS][COLS]);
static void read_input(Game*game);
static void init_game(Game*game);
static int astar_state(Game*game,const int sr[],const int sc[],int len,int step0,int dir0,
	int tgtr,int tgtc,int pathr[],int pathc[],int*fdir);
static int sim_follow_state(Game*game,const int in_sr[],const int in_sc[],int in_len,int step0,
	int food_tr,int food_tc,int pathr[],int pathc[],int plen,int out_sr[],int out_sc[],int*out_len,
	int*out_dir);
static int state_space(const Game*game,const int sr[],const int sc[],int len);
static int state_options(const Game*game,const int sr[],const int sc[],int len,int dir0,
	int step0);
static int state_wall_touch(const Game*game,const int sr[],const int sc[],int len);
...
static int sim_follow_global(Game*game,int pathr[],int pathc[],int plen);
static int sim_safe_like(const Game*game,int min_space);
static int flood_eval(const Game*game,int startr,int startc,int blk[ROWS][COLS],int tailr,
	int tailc,int*tail_ok);
static int choose_direction_low(Game*game);
static int safe_after_path(Game*game,const int in_sr[],const int in_sc[],int in_len,int step0,
	int food_tr,int food_tc,int pathr[],int pathc[],int plen);
static int sim_one_step(const Game*game,int d,int out_sr[],int out_sc[],int*out_len,
...
static int should_use_slow_high(const Game*game);
static int should_finish_slow_high(const Game*game,int elapsed);
...
static HNode hp_pop(Game*game){
	...
	while(i*2<=game->hpsz){
		ch=i*2;
		if(ch+1<=game->hpsz&&game->hp[ch+1].f<game->hp[ch].f){
			ch++;
		}
	...
	*fdir=-1;
	...
	while(game->hpsz>0){
		...
		if(r==tgtr&&c==tgtc){
			...
			for(d=0;d<4;d++){
				if(hr+dir_row_delta(d)==pathr[0]&&hc+dir_col_delta(d)==pathc[0]){
					*fdir=d;
					break;
				}
			}
		...
		for(d=0;d<4;d++){
			...
			if(ng<gs[nr][nc]){
				gs[nr][nc]=ng;
				from[nr][nc]=d;
				hp_push(game,nr,nc,ng+mhdist(nr,nc,tgtr,tgtc));
			}
		}
	}
	return 0;
...
static int sim_follow_state(Game*game,const int in_sr[],const int in_sc[],int in_len,int step0,
	...
	*out_dir=-1;
	for(step=0;step<plen;step++){
		...
		out_sc[0]=nc;
		for(i=0;i<4;i++){
			if(pr+dir_row_delta(i)==nr&&pc+dir_col_delta(i)==nc){
				*out_dir=i;
				break;
			}
		}
	...
	for(i=0;i<ROWS;i++){
		for(j=0;j<COLS;j++){
			...
			for(d=0;d<4;d++){
				...
				if(nr>=0&&nr<ROWS&&nc>=0&&nc<COLS&&!game->wallmap[nr][nc]){
					degree[i][j]++;
				}
			}
		}
...
static int sim_follow_global(Game*game,int pathr[],int pathc[],int plen){
	int lastdir;
	return sim_follow_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->foodr,
		game->foodc,pathr,pathc,plen,game->tmp_sr,game->tmp_sc,&game->tmp_slen,&lastdir);
}
static int sim_safe_like(const Game*game,int min_space){
	int blk[ROWS][COLS];
	int vis[ROWS][COLS];
...
static int choose_direction_low(Game*game){
	...
	safe_food_dir=-1;
	fdir=-1;
	plen=astar_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->curdir,game->foodr,
		game->foodc,pathr,pathc,&fdir);
	if(plen>0&&fdir>=0){
		if(game->nval<=4&&sim_follow_global(game,pathr,pathc,plen)){
			return fdir;
		}
		...
		if(safe_food_dir<0&&static_dead_cell(game,game->foodr,game->foodc)&&sim_follow_global(game,
			pathr,pathc,plen)){
			return fdir;
		}
	}
	...
	if(game->nval>1){
		tdir=-1;
		astar_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->curdir,
			game->srow[game->slen-1],game->scol[game->slen-1],pathr,pathc,&tdir);
		if(tdir>=0){
			tail_chase_dir=tdir;
		}
	}
	...
	for(d=0;d<4;d++){
		...
		if(game->bodyid[nr][nc]>=0){
			int istail;
			istail=(nr==game->srow[game->slen-1]&&nc==game->scol[game->slen-1]);
			if(!(istail&&!growing)){
				continue;
			}
		}
		...
		if(growing){
			blk[game->srow[game->slen-1]][game->scol[game->slen-1]]=1;
		}
		blk[nr][nc]=0;
		if(growing){
		...
		fdist=(game->foodr>=0)?mhdist(nr,nc,game->foodr,game->foodc):40;
		val=ff;
		if(game->nval>1&&tok){
			val+=5000;
		}
	...
	if(bestd<0){
		for(d=0;d<4;d++){
			...
			int dummy_sr[MAXSN];
			int dummy_sc[MAXSN];
			if(sim_one_step(game,d,dummy_sr,dummy_sc,&dummy_len,&eating,&growing)){
				bestd=d;
				break;
			}
		}
...
static int safe_after_path(Game*game,const int in_sr[],const int in_sc[],int in_len,int step0,
	...
	if(!sim_follow_state(game,in_sr,in_sc,in_len,step0,food_tr,food_tc,pathr,pathc,plen,
		game->tmp_sr,game->tmp_sc,&game->tmp_slen,&lastdir)){
		return 0;
	}
	if(lastdir<0){
		return 0;
	...
	end_step=step0+plen;
	space=state_space(game,game->tmp_sr,game->tmp_sc,game->tmp_slen);
	opts=state_options(game,game->tmp_sr,game->tmp_sc,game->tmp_slen,lastdir,end_step);
	if(game->nval<=1){
		need=6+plen/2;
	...
	dummy=-1;
	tail_len=astar_state(game,game->tmp_sr,game->tmp_sc,game->tmp_slen,end_step,lastdir,
		game->tmp_sr[game->tmp_slen-1],game->tmp_sc[game->tmp_slen-1],backr,backc,&dummy);
	if(tail_len<=0){
		return 0;
	}
	if(game->nval<=2){
...
static int direction_to_cell(const Game*game,int r,int c){
	int d;
	for(d=0;d<4;d++){
		if(game->srow[0]+dir_row_delta(d)==r&&game->scol[0]+dir_col_delta(d)==c){
			return d;
		}
	}
	return-1;
...
static int follow_cached_plan(Game*game){
	...
	if(game->plantargetr!=game->foodr||game->plantargetc!=game->foodc){
		return-1;
	}
	d=direction_to_cell(game,game->planr[game->planpos],game->planc[game->planpos]);
	if(d<0){
		return-1;
	}
	if(!sim_one_step(game,d,nsr,nsc,&nlen,&eating,&growing)){
		return-1;
	}
	game->planpos++;
	return d;
...
static int choose_direction_fast(Game*game){
	...
	for(d=0;d<4;d++){
		...
		opts=state_options(game,nsr,nsc,nlen,d,game->stepcnt+1);
		wall_touch=state_wall_touch(game,nsr,nsc,nlen);
		fdist=(game->foodr>=0)?mhdist(nsr[0],nsc[0],game->foodr,game->foodc):40;
		val=space*55+opts*500-wall_touch*160;
		val+=(40-fdist)*55;
		val-=game->visitcnt[nsr[0]][nsc[0]]*((game->nval>=128)?260:170);
		if(space<=4){
			val-=20000;
		}
		else if(space<=8){
...
static int should_finish_fast(const Game*game,int elapsed){
	...
	if(game->nval>=512){
		return game->stepcnt>=6200&&game->score>=1700;
	}
	if(game->nval>=256){
		return game->stepcnt>=6600&&game->score>=2200;
	}
	if(game->nval>=128){
		return game->stepcnt>=7200&&game->score>=1800;
	}
	if(game->nval>=64){
		return game->stepcnt>=7600&&game->score>=1600;
	}
	return 0;
}
static int should_use_slow_high(const Game*game){
	return(game->nval==64||game->nval==512)?1:0;
}
static int should_finish_slow_high(const Game*game,int elapsed){
	if(elapsed>=930){
		return 1;
...
static int choose_direction(Game*game){
	...
	slow_high=should_use_slow_high(game);
	if(slow_high&&should_finish_slow_high(game,elapsed)){
		return choose_finish_direction(game);
	}
	if(!slow_high&&should_finish_fast(game,elapsed)){
		return choose_finish_direction(game);
	}
	if(game->nval<=1){
		return choose_direction_low(game);
	}
	if(!slow_high&&((game->nval>=512&&game->stepcnt>7000)||(game->nval>=256&&elapsed>320)||
		(game->nval>=128&&elapsed>340))){
		return choose_direction_fast(game);
	}
	if(!slow_high&&(game->nval>=128||game->nval==4)){
	...
	if(game->foodr>=0){
		...
		int food_len;
		fdir=-1;
		food_len=astar_state(game,game->srow,game->scol,game->slen,game->stepcnt,game->curdir,
			game->foodr,game->foodc,pathr,pathc,&fdir);
		if(food_len>0&&fdir>=0&&safe_after_path(game,game->srow,game->scol,game->slen,game->stepcnt,
			game->foodr,game->foodc,pathr,pathc,food_len)){
			int direct_need;
			int direct_space;
			if(game->nval<=16){
				direct_need=10;
			...
			direct_space=state_space(game,game->tmp_sr,game->tmp_sc,game->tmp_slen);
		...
		if(food_len>0&&fdir>=0&&(static_open_degree(game,game->foodr,game->foodc)<=1||
			static_dead_cell(game,game->foodr,game->foodc))&&sim_follow_global(game,pathr,pathc,
			food_len)){
			game->planlen=food_len;
			game->planpos=1;
			game->plantargetr=game->foodr;
	...
	for(d=0;d<4;d++){
		...
		if(game->nval>1){
			dummy=-1;
			tail_len=astar_state(game,nsr,nsc,nlen,game->stepcnt+1,d,nsr[nlen-1],nsc[nlen-1],pathr,pathc,
				&dummy);
		}
		food_len=0;
		food_safe=0;
		if(!eating&&game->foodr>=0){
			dummy=-1;
			food_len=astar_state(game,nsr,nsc,nlen,game->stepcnt+1,d,game->foodr,game->foodc,pathr,pathc,
				&dummy);
			if(food_len>0){
				food_safe=safe_after_path(game,nsr,nsc,nlen,game->stepcnt+1,game->foodr,game->foodc,pathr,
					pathc,food_len);
			}
		}
		fdist=(game->foodr>=0)?mhdist(nsr[0],nsc[0],game->foodr,game->foodc):40;
		...
		if(tail_len>0){
			val+=21000+(tail_len<80?tail_len:80)*44;
		}
		else{
			val-=3000;
		}
		if(food_safe){
			if(slow_high){
				...
				val+=32000-food_len*((fill_level>60)?55:25);
			}
		}
		else if(food_len>0){
			val+=slow_high?(2200-food_len*12):(3200-food_len*8);
		}
		if(eating){
		...
		val+=(40-fdist)*12;
		val-=game->visitcnt[nsr[0]][nsc[0]]*((game->nval<=2)?35:((game->nval<=16)?70:220));
